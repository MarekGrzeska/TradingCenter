...
|---|---|---|---|
| **high** | Terraform state, not this diff | The first plan on PR #201 read `6 to add, 4 to change, **38 to destroy**`, and all 38 destroys were `polymarket-data`'s production footprint: 32 firewall rules, its App Service, the `polymarket` database, three Easy Auth resources and a Key Vault policy. That module had been applied to production from a branch not yet on `main`, so it lived in state and not in the code any plan from `main` reads. Applying this branch as it then stood would have deleted a running module and its database. Nothing in this change caused it — it was the first PR to touch `infra/` since that apply, so the first plan to say so. | FIXED — `polymarket-data-joins-the-stack` merged as #200 and this branch rebased onto it; the destroys are gone from the plan. The lesson outlives the fix and is in `design.md`'s Migration Plan: an apply from an unmerged branch turns every later plan into a request to delete it. |
