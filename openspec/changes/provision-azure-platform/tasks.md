...
- [x] 5.7 **(dawne 4.7)** Rola aplikacyjna w Postgresie: `pgaadauth_create_principal_with_oid('app-tradingcenter-market-data', <object_id>, 'service', false, false)` — sygnatura funkcji ma inną kolejność argumentów niż w 4.8 (`rolename, objectid, objecttype, isadmin, ismfa`, nie `oid, rolename, isadmin`); `CONNECT`+`CREATE`/`USAGE` wyłącznie na `market_data`, jawny `REVOKE ALL` na `market_data_dev`. **Sprawdzone ręcznie, częściowo**: `has_database_privilege` z konta administratora potwierdza `CONNECT`=true na `market_data`, false na `market_data_dev`. Token samej tożsamości zarządzanej nie do zdobycia z zewnątrz — `IDENTITY_ENDPOINT`/`IDENTITY_HEADER` istnieją tylko w procesie kontenera aplikacji, nie w konsoli Kudu/SCM, a kod jeszcze nie jest wdrożony (grupa 7). **Weryfikacja domknięta 2026-08-09 przy pierwszym wdrożeniu**: wdrożony `market-data` połączył się z `market_data` własną tożsamością zarządzaną i wykonał zapytanie — dowodem jest to, że poległ dopiero na `UndefinedTableError`, czyli po uwierzytelnieniu. Token tożsamości zarządzanej zadziałał tak, jak zakładano
...
- [ ] 7.4 Pierwsze wdrożenie gatewaya i sprawdzenie całego łańcucha na module, który już działa — **nadal wstrzymane, ale z innego powodu niż zapisany pierwotnie**. Pierwotna notatka mówiła, że blokadą jest filtr `ref:refs/heads/main` w poświadczeniu federowanym; po realnym pushu brancha okazało się, że blokada była twardsza i dotyczyła też PR-ów: GitHub przeszedł na **niezmienne subjecty OIDC** i prezentuje `repo:MarekGrzeska@48219464/TradingCenter@1326647472:...`, czego nie dopasowywało żadne z dwóch poświadczeń — Entra odrzucała każdy token (`AADSTS700213`), objawiając się jako `terraform init` wiszący 7,5 minuty. Naprawione (`infra/github-oidc.tf`, obie formy subjectu zarejestrowane). Samo wdrożenie aplikacji dalej czeka na scalenie do `main`
...
- [x] 7.8 Workflow Terraforma (`terraform.yml`): `plan` na pull requestach (komentarz z planem). `infra/terraform.tfvars` jest per-operator i gitignored — te same pięć wartości powielone jako `TF_VAR_*` w GitHub vars, bo CI nie jest operatorem z własnym `.tfvars`. **`apply` po push do `main` usunięty, wbrew pierwotnemu planowi tego zadania**: pierwszy realny bieg pipeline'u pokazał, że ten root nigdy nie był z CI ani applyowalny, ani nawet planowalny, i że udany apply byłby gorszy niż nieudany — `azurerm_key_vault_access_policy.operator` i `azurerm_role_assignment.operator_tfstate` brały principal z `data.azurerm_client_config.current`, czyli z tego, *kto uruchamia Terraform*, więc apply z CI skasowałby dostęp operatora do Key Vaulta i przepisał go na tożsamość pipeline'u. Obie czytają teraz jawne `var.operator_object_id`. Do samego planu CI dostało minimum: `Application.Read.All` w Graph (bez tego 403 na trzech aplikacjach Entra) i `Reader` na `rg-tradingcenter-tfstate` (bez tego 403 na odczycie przypisań ról); `data.azurerm_storage_account.tfstate` wycięty, bo to odczyt control-plane, a CI ma tam wyłącznie rolę data-plane. Apply zostaje u operatora — wdrożenia aplikacji dalej idą przez OIDC, więc decyzja z design.md „Wdrożenia przez OIDC" zostaje w mocy, a infrastruktura jest jej nazwanym wyjątkiem
...
- [x] 8.6 Migracje na `market_data_dev` kontem deweloperskim, potem na `market_data` kontem aplikacyjnym — **dev zweryfikowane w pełni, na żywo**: `alembic upgrade head` przez `sp-tradingcenter-market-data-dev` (token Entra, TLS) przeszło na `market_data_dev`; ta sama rola na `market_data` dostała `InsufficientPrivilegeError: User does not have CONNECT privilege` (spójne z 4.11). **Strona produkcyjna domknięta 2026-08-09**: migracje na `market_data` puszczone kontem administratora z `ALTER ROLE ... IN DATABASE market_data SET role`, żeby właścicielem ośmiu tabel została rola aplikacji, a nie operator — tak samo jak na dev, gdzie właścicielem jest rola, która bazy używa. Alembic na `0006`, ustawienie roli cofnięte po migracji. Procedura zapisana w `docs/rotacja-poswiadczen.html`, bo tożsamości zarządzanej nie da się użyć spoza App Service i za pół roku nikt tego nie odtworzy z pamięci. Po drodze poprawka w `migrations/env.py`: SQLAlchemy przekazuje `sslmode` z URL jako dosłowny kwarg do `asyncpg.connect()`, którego ten nie rozumie (`ssl=`, nie `sslmode=`) — asyncpg samo parsuje `sslmode` z surowego DSN, więc `pool()`/`connect()` w `db.py` nigdy na to nie trafiły
...
- [x] 11.4 Zweryfikuj wdrożoną platformę end-to-end: logowanie do terminala, wykres z archiwum, zapis świec przez ingest — **niewykonalne w tej zmianie i zrobione w następnej.** Terminal i `market-data` stoją pod różnymi adresami, więc przeglądarka nie dokłada ciasteczka Easy Auth, a strumień świec nie ma gdzie nieść tokenu, bo przeglądarkowy WebSocket nie przyjmuje nagłówków. Rozstrzygnięcie i wybór drogi: `docs/terminal-market-data-auth.html`; realizacja: zmiana `authenticate-terminal-to-market-data`, której grupa 4 potwierdza wszystkie trzy rzeczy z tego zadania na wdrożonej platformie — logowanie kontem organizacji, `WebSocket /ws/candles?…&ticket=… [accepted]` z wykresem rysującym świece, oraz ingest zapisujący do bazy w Azure
