...
- [x] 3.8 **Godzina na B2** (6 września 2026, 20:45–21:50 UTC, niedziela — z rynków otwarty tylko BTC; przerwa 148 s do odpowiedzi czterech aplikacji; `MemoryPercentage` 79–83%, maksimum 83%; zero restartów; working sety: workbench 394–425 MB, gateway 291–341, trading-mcp 162–171, telegram 217–228) — spokojna godzina z otwartym rynkiem, `az appservice plan update --sku B2`, `MemoryPercentage` co 5 minut, które aplikacje restartowały, powrót tą samą komendą z `B3`; wynik do design.md jako liczba zamiast dwóch kolumn
- [x] 3.9 Decyzja z 3.8: **< 85% → B2 zostaje, etap 4 pominięty, dalej etap 5**; Terraform poznaje `sku_name = "B2"` w 5.1 — do tego czasu plan jest zmieniony poza stanem, świadomie — < 85% → etap 5; 85–100% → etap 4; restarty/OOM → B3 zostaje, etap 5 bez SKU, powód przy `sku_name` w `app-service.tf`
...
- [x] 5.4 Dokumenty: CLAUDE.md (reguła nośna, tabela, porty, „Things that will bite you"), `docs/architecture.md`, `docs/mniej-modulow-czy-aks.html` do `docs/archive/` z notą o zmienionej liczbie, pięć przewodników → jeden — 7 września 2026: reguła nośna w CLAUDE.md brzmi „one process per security boundary" (tabela i porty były aktualne od 3.7), „The shape" w `docs/architecture.md` rysuje cztery procesy, AKS-owy dokument w archiwum z notą w nagłówku, `docs/przewodnik-po-modulach.html` zastępuje pięć przewodników z 20 sierpnia (te w archiwum)
