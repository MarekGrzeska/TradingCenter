...
A twin of `market_data/migrate.py`, duplicated rather than shared — no library between
...
a merge and a working module (`agent-database-connection`, "Moduł sam doprowadza bazę do
...
Service picks — the same resolution that made the last hand-run migration need a copy of
that file with absolute paths in it.
"""
