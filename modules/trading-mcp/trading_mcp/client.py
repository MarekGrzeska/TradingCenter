...
def _detail(response: httpx.Response) -> str:
    ...
    validation objects. Both are capital-gateway's own words and both travel as a sentence.
    ...
    try:
        ...
        return response.text.strip() or f"capital-gateway refused with HTTP {response.status_code}"
...
def _one_problem(entry: object) -> str:
    ...
    (specs/trading-mcp-tools, "Odmowa nazywa, co zmienić"). The first element of `loc` is FastAPI's own plumbing
