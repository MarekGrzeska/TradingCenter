...
/**
 * Merge one bar into a series sorted by `time` with no duplicate timestamps. The two cases every live tick
 * takes — replacing the last bar, appending after it — touch only the tail; anything else binary-searches.
 */
...
/**
 * Two series into one, `incoming` winning a shared period. A single linear pass, not `mergeBar` per bar: that
 * copies the whole array each time, several million element copies for a 300-candle page — a visible stutter.
 */
