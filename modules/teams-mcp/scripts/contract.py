"""teams' wire, snapshotted rather than assumed.

    uv run python scripts/contract.py generate   # rewrite contract/teams.openapi.json
...
market-data, teams prints its own document (`python -m teams.openapi`), so this reads that
rather than importing anything: `Settings()` is built inside `lifespan`, never at import
time, so the schema is a property of contract.py's models and the routes alone — no
database, no OpenAI key, no market-mcp.
"""
...
TEAMS = MODULE_ROOT.parent / "teams"
OUTPUT = MODULE_ROOT / "contract" / "teams.openapi.json"

# The floor of teams' own `requires-python`, pinned for the same reason market-mcp
# pins market-data's: a newer interpreter can change a stdlib-sourced
...
def schema_text() -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="teams-mcp-contract-") as scratch:
        env["UV_PROJECT_ENVIRONMENT"] = str(Path(scratch) / "python-env")
        try:
            result = subprocess.run(
                ["uv", "run", "--python", PYTHON_VERSION, "python", "-m", _PRINT_MODULE],
                cwd=TEAMS,
                capture_output=True,
                text=True,
                encoding="utf-8",
...
def check() -> None:
    ...
    if committed != fresh:
        print(
            f"{OUTPUT.relative_to(MODULE_ROOT)} does not match teams' schema.\n"
