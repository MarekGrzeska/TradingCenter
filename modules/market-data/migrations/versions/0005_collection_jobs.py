...
# Mirrors `Settings.default_backfill_bars`'s own default. A pair tracked before this
# column existed was reaching back this many candles on its first fill — this migration
# gives it a `collect_from` that says exactly that, rather than leaving the column blank
# for rows nobody will ever fill in by hand.
DEFAULT_BACKFILL_BARS = 5_000

# Chunk states. `interrupted` is reached only from `pending` or `running`, and only by the
# module's own startup — no runner survives a restart, so anything not yet `done`,
# `failed` or `skipped` at that moment is orphaned, not merely delayed.
...
def upgrade() -> None:
    op.create_table(
        ...
        # What the operator asked for, before any per-pair clipping to provider history.
        # A chunk's own window is the clipped, authoritative one; this is kept beside it
        # so a job can still say what was requested versus what was actually reachable.
        sa.Column("requested_from", sa.TIMESTAMP(timezone=True), nullable=False),
        # Bumped by a retry. Kept on the job rather than inferred from its chunks, because
        # "how many times has this been retried" is a fact about the job, not a count that
        # should have to be recomputed by scanning every chunk's own attempt number.
    ...
    op.create_table(
        ...
        # The window this one chunk covers — one gateway request's worth, per
        # `MAX_BARS_PER_FILL`. Half-open in spirit (start inclusive, end exclusive), matching
        # every other range in this module.
        ...
        # Which attempt of the *job* last touched this chunk. A chunk done on attempt 1
        # stays done through a later retry of its siblings — this is what lets a retry
        # skip it rather than re-asking the provider for a range it already has.
        sa.Column("attempt", sa.Integer(), nullable=False, server_default="1"),
        ...
        # A pair collection_job_chunks names must be one tracking has actually decided on
        # — untracking flips a row rather than deleting it, so this holds even for a pair
        # stopped after its job ran.
