...
from market_data.schema_version import SchemaMismatch, applied_heads, expected_heads, verify
...
async def test_a_database_one_migration_behind_refuses_to_start() -> None:
    with pytest.raises(SchemaMismatch) as err:
        await verify(FakeConnection(["0006_pair_deletions"]))  # type: ignore[arg-type]
