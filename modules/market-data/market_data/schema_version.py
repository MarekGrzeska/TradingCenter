...
A twin of `agent/schema_version.py`, duplicated rather than shared. A comparison, never a
...
- **database ahead of the image** — an older image was deployed onto a newer schema. This
  one gets *more* likely once deployments migrate on their own, not less, because the
  schema now moves forward at every deploy and a rollback moves only the code back.
...
Written after 10 August, when a deploy carrying migration `0007` reported success against
a database still at `0006` and four routes answered `500` for thirty-five minutes while
the deploy sat green.
"""
