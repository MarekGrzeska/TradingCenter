...
A twin of `agent/migrate.py`, duplicated rather than shared — no library between modules.
The container runs this at startup (`app.py`), under the advisory lock in `db.py`, so a
deployment carries its schema with it and no operator step stands between a merge and a
working module (`market-data-database-connection`, "Moduł sam doprowadza bazę do rewizji,
dla której powstał").
...
def alembic_config(database_url: str | None = None) -> Config:
    ...
    throwaway PostgreSQL. Left out, `env.py` reads the module's own settings and connects
    with its identity, which is the production path.
    """
    config = Config()
