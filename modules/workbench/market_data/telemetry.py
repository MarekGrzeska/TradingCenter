...
Logging and the exporter are the host's (`tc_runtime.telemetry`); this file only publishes."""
...
# weeks, measured 4 September 2026. Read once, when the FastAPI instrumentor is first imported, so the host
# puts this under `OTEL_PYTHON_FASTAPI_EXCLUDED_URLS` before it configures telemetry; the path is the one
# within the mount, and the host prefixes it (`workbench/app.py`).
UNTRACED_PATH = "/ws/candles"
