...
**A package of the workbench since 5 September 2026** (`one-process-per-security-boundary`, stage 3):
`market_data/` is mounted whole under `/market`, so every path below is read with that prefix in
front, the process that serves it is `uvicorn workbench.app:app` on 8030, and its settings are read
under `MARKET_` beside the process's own gateway quartet (`modules/workbench/.env.example`). Nothing
about what it archives, serves or refuses changed with the move; `python -m market_data.openapi`
still prints its contract, from `modules/workbench`.
