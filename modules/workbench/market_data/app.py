...
under `/market` and enters `serving` from its own lifespan, because a mounted application's lifespan is never
run — and the host configures telemetry, which this file did at import while it was a process of its own.
`app` and `lifespan` below are what `python -m market_data.openapi` and the tests build.
"""
...
# What `python -m market_data.openapi` prints from. The process that serves it is the workbench's.
app = create_app()
