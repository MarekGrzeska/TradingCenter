...
def current_principal(request: Request) -> str:
    """A `Depends()` used by every owned-resource route — raising here refuses a request before it reaches
    a route body, so the requirement refuses before a model is ever touched."""
