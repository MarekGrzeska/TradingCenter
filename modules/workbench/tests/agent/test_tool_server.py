"""The conversation's MCP client against a real MCP server, since `trading-mcp` is not importable from here. Slower than
...
def _stand_in_server(port: int) -> FastMCP:
    ...
    def get_last_price(symbol: str) -> str:
        if symbol != "US100":
            # The shape trading-mcp refuses in: a sentence naming what to change. Raising
    ...
    # trading-mcp's real `list_tracked_pairs` returns a typed list, not a string. The SDK turns a bare list
...
async def test_a_slow_server_times_out_as_unavailable() -> None:
    ...
    async with _serving(mcp.streamable_http_app(), port):
        client = ToolServer(
            settings_for(f"http://127.0.0.1:{port}", trading_mcp_request_timeout_seconds=1.0)
        )
        try:
...
async def test_no_configured_server_means_no_tools_and_no_calls() -> None:
    ...
    assert "trading-mcp" in outcome.text
...
async def test_a_read_on_the_same_server_is_still_unavailable() -> None:
    ...
    so a failed one carries the same "nothing happened" trading-mcp's does."""
    client = ToolServer(
        settings_for(None, trading_mcp_url=f"http://127.0.0.1:{_free_port()}"),
        prefix="trading_mcp",
        can_move_the_account=True,
...
async def test_a_server_that_cannot_move_the_account_never_answers_unknown() -> None:
    """A server built without the flag: every failure there is still `unavailable`,
