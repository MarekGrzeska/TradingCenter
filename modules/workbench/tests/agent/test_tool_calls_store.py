...
class ServerWithNoTools(FakeToolServer):
    """trading-mcp unconfigured or unreachable — `list_tools` answers with nothing rather than raising,
...
async def test_a_turn_without_tools_runs_the_prompt_that_says_so(pool, db) -> None:
    # The prompt is picked from what the turn actually has, not from configuration: a trading-mcp that was
    ...
    # This module's own three tools do not need trading-mcp to exist, so they are still offered; the prompt
