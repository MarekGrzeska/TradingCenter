...
# What trading-mcp puts on every one of its tools. The stand-in carries it for the same reason it is a real
...
def _register(mcp: FastMCP, name: str) -> None:
    ...
    if name == "get_last_price":
        ...
        # trading-mcp could give, and a test pins that it travels as "unknown" rather than as a guess.
        @mcp.tool(name=name, description="Returns the last price for a symbol, in UTC, bid side.")
        def get_last_price(symbol: str) -> str:
            if symbol != "US100":
                # The shape trading-mcp refuses in: a sentence naming what to change. Raising is how a
