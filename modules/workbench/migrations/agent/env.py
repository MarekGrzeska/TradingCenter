"""How alembic reaches the database — duplicated from `market_data/migrations/env.py`
rather than imported; see `agent/db.py`'s own docstring for why."""
...
if config.config_file_name is not None:
    # `disable_existing_loggers=False` — see market_data's twin: alembic runs in-process
    # during tests, and the default would silently kill every `agent.*` logger already
    # created by importing the module.
...
async def run_migrations_online() -> None:
    ...
    if credential is not None:
        ...
        # string as literal kwargs to asyncpg.connect(), which does not accept
        # `sslmode=` — carried into connect_args as `ssl` instead.
